...
public:
	...
	void Initialize();
	bool SetHeight(float height, float speed, float tolerance = 1);
	bool SetAngle(float angle, float speed, float tolerance = 3);
	bool SetRoller(float speed);
	void SetNotePosition(float position);
